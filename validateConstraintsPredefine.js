...
module.exports = function(withService) {
    if(withService){
        constraints.sessionIdConstrOptional = withServiceSessionIdConstr;
        constraints.sessionIdConstrRequested = _.extend({presence: true}, withServiceSessionIdConstr);

        constraints.ExcludeSpecialCharactersOptional = withServiceAllowedChars;
        constraints.ExcludeSpecialCharactersRequested = _.extend({presence: true}, withServiceAllowedChars);

        constraints.freeFieldOptional = withServiceAllowedChars;
        constraints.freeFieldRequested = _.extend({presence: true}, withServiceAllowedChars);

        constraints.openTextConstrOptional = withServiceAllowedChars;
        constraints.openTextConstrRequested = _.extend({presence: true}, withServiceAllowedChars);

        constraints.passcodeConstrOptional = withServicePassword;
        constraints.passcodeConstrRequested = _.extend({presence: true}, withServicePassword);
    }
    return constraints;
}
